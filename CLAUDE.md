...
`middlewares/license_gate.py` — блокирует хендлеры в RESTRICTED режиме. **Сейчас не зарегистрирован** в `bot.py` — 3 строки закомментированы с `# TODO` рядом с `evaluate_license`. Включить при ~20 клиентах.
...
- **Коммиты**: не приоритет, проект локальный — не навязывать. Но для Phase-работы из `senior-upgrade-prompt.md` — по коммиту на фазу, это в брифе.
...
- **Phase 5** ✅ Ed25519-лицензии: `utils/license.py`, `middlewares/license_gate.py` (**enforcement off**, см. выше), `utils/heartbeat.py`, `tools/generate_keys.py`, `tools/issue_license.py`, `docs/LICENSING.md`. Grace 90 дней.
