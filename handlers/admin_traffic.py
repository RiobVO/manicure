...
async def cb_traffic_qr(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(
        ...
        caption=(
            f"📱 <b>{src['label']}</b>\n"
            f"код: <code>{src['code']}</code>\n\n"
            f"<b>как распечатать:</b>\n"
            f"• <i>с телефона:</i> нажми и удерживай картинку → «Сохранить в галерею»\n"
            f"• <i>с компьютера:</i> правый клик по картинке → «Сохранить как»\n\n"
            f"печатай на A5, оставь белые поля сверху и снизу — QR считывается с метра."
        ),
        parse_mode="HTML",
