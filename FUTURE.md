...
- **Онлайн-оплата через бота (Click/Payme).** Код готов и протестирован моком,
  провайдер отключён через `PAYMENT_PROVIDER=none` в `.env.template` — новые
  салоны по умолчанию работают без онлайн-оплаты (оплата на месте как обычно).
  Триггеры для активации: (а) салон попросил предоплату, (б) статистика no-show
  превысила 15% и предоплата станет рычагом. Флоу включения: получить
  merchant-аккаунт у Click/Payme, проставить креды (`CLICK_*` / `PAYME_*`)
  в `.env`, поменять `PAYMENT_PROVIDER` на `click` или `payme`, прогнать тест
  на малой сумме, затем прод. Весь код остаётся в репе dormant — переключение
  одного флага.
